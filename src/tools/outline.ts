...
export function registerOutlineTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		label: "Outline",
		description:
			"Outline one known file with CodeMapper after map/search has identified it. Use only for CodeMapper-supported languages/docs: Python, JavaScript/TypeScript, Rust, Java, Go, C, Swift, Ruby, and Markdown; for unsupported languages such as Emacs Lisp, prefer rg/read. For code files, returns file metadata plus symbols with names, types, line ranges, signatures, and exported flags when available. For Markdown files, returns h1-h3 heading sections with line counts. Use before reading a full file to choose targeted ranges; this is not repo-wide symbol search.",
		promptSnippet: "outline: Show one file's code symbols or Markdown h1-h3 sections before targeted reads; returns JSON items.",
		promptGuidelines: [
			"Use CodeMapper outline only for supported languages/docs: Python, JavaScript/TypeScript, Rust, Java, Go, C, Swift, Ruby, and Markdown; for unsupported languages such as Emacs Lisp, use rg/read instead.",
